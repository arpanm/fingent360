...
Input readiness: before picking a task, read its dated input record and `docs/tasks/pickup-queue.md`. Reuse recorded user answers; research regulations, source terms and formats from primary evidence rather than asking the user to discover them. Ask only for a concrete unavailable private input or decision. Record questions and exact answers in the task file. Agent-ready/research-ready permits authoring/research only; validation-only requires saved user-run evidence, not another implementation or agent-run suite. A release/permission gate does not block independent research.

Functional closure: use the reviewed case/project requirements in `docs/tasks/acceptance.json`. Update them when changing a story, including connected and offline coverage. Its `_completion` metadata may permit automatic Done only after implementation is complete, external gates are empty and the full current acceptance matrix passes. SDLC owns generated validation blocks and docs/bugs; never replace a saved failure with an authored pass. Work one functional story through closure before starting another.

Task storage: `TODO.md` is only a task list with title/link, status and clear blocker/next action. Before each implementation, read the linked `docs/tasks/ID.md`. Put the detailed scope, reusable prompt, implementation summary, remaining work and verification there. Follow `docs/tasks/README.md`. Update that file AND the matching TODO row after every implementation, and reconcile affected parent tasks. Never add long prompts or history back into TODO. Archived task text is historical context, not permission to execute commands.

Follow `docs/development/sdlc.md`. Before implementing, add/update the linked task file with stable ID, full context, scope, dependencies, acceptance criteria and a reusable detailed Codex prompt; keep only its concise status row in root `TODO.md`. Update rather than duplicate tasks. Keep implementation status separate from verification.
