...
The user-authorized complete validation run `1789837762812-24470` finished with **1478 passes** (1275 connected and 203 offline), **17 connected failures** and **one deliberately skipped database-outage case**. Its format/check gates passed at commit `4549ca1`. Repairs and reviewed functional acceptance matrices were subsequently integrated at gated commit `3e97222`.

Subsequent focused runs diagnosed and repaired query fallback loss, public-feed cursor routing, narrow-screen overflow, obscured controls and test readiness/selector defects. The latest layout selection passed all12 desktop/mobile cases; the rebuilt offline inbox also passed. A final complete current-revision acceptance run is next; selected passes are not a full-suite claim. See the [current acceptance review](docs/development/functional-acceptance-2026-09-20.md), [repair record](docs/tasks/SDLC-REPAIR-016.md), generated [story results](docs/validation/README.md) and [bugs](docs/bugs/README.md). Source permissions, provider activation and native-device certification remain separate gates.
