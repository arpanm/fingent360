...
The active functional story is GOALS-001. Confirmed removal now closes the edited goal's wizard and clears its navigation guard; cancelling removal or deleting another goal preserves the draft. Delayed older list replies cannot restore removed goals. New WEB068/069 and OFFLINE068 cases use real server/on-device storage. Full acceptance command: `pnpm sdlc "Complete saved goals" --story GOALS-001`. Changes are authored, not yet validated.
