...
The complete validation at `ff2ab59` recorded **1317 connected passes, one failure and one deliberate outage skip**, plus **208 offline passes**. The only remaining failure was a daily-oil form hydration race: a late initial response could erase typed permission evidence. The repair at `c8e4bb8` passed its desktop and mobile regression; the separately executed database-outage case also passed. MongoDB was restored and API readiness returned200 after restarting its existing watcher child.

These receipts belong to their recorded revisions. The validation ledger currently uses a repository-wide fingerprint, so the isolated repair makes unrelated previous passes stale; it does not certify them for the new revision. See the [current acceptance review](docs/development/functional-acceptance-2026-09-20.md), [remaining gates and scope](docs/development/validation-remaining-2026-09-20.md), generated [story results](docs/validation/README.md) and [bugs](docs/bugs/README.md). Source permissions, provider activation and native-device certification remain separate gates.
