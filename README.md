...
Current functional repairs address source capture → independent publication → public brief/scenario prerequisites, browser Operations actor switching, and stable broker/action-comparison selectors shared by web and Android. Existing real API/storage and rejection checks are retained. These edits await manual validation; no new source permission, broker parser, deployment or production-readiness claim is made. Focused commands and case IDs: [functional acceptance repairs](tests/e2e/CATALOG.md#functional-acceptance-repairs--2026-09-17).
