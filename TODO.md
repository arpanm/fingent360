...
- **READING-FOLLOW-001 (DEV-018): Pending after SOURCE-WITHDRAWAL-001.** Explicit source/topic subscriptions with a manual check of newly reviewed stored editions, dated coalesced private inbox, mute/acknowledge/reopen and withdrawal-safe navigation. No provider refresh, continuous monitoring, email/push or urgency/material-impact inference. Before authoring, expand the child specification and Codex prompt with subscription editions, consent/retention/idempotency/ownership, current-versus-historical context, server and actual local storage, export/deletion, API/browser/offline acceptance. Source/withdrawal and library/privacy prerequisites must be integrated first. Reserve API440–459, WEB440–459, OFFLINE450–469; additive032 only if needed. The broader event-policy and external-delivery gates remain open.
...
- **Verification:** AUTH-WAIT has passing evidence for all32 selected connected scenarios across correction runs, including all12 new authorization selections, plus107 unit tests; final gated commit follows. Remaining children are unverified until parent integration. Completed REPORTS-002 is c141414; XLSX-001 is dec6d47 with107 unit tests and passing connected/offline evidence in status. Other next-child candidates are a durable research-connection review inbox and research-connected report snapshots; these have not been implemented or marked complete.
...
- **Status/dependencies:** Implemented guided existing holding/goal storage and three configurable provider adapters plus query fallback. Live paid-provider validation and broader verified valuation/connected allocations remain separate open dependencies. Parents DEV-007–009/016/017/019.
...
- **Implementation:** Android feedback APK implemented in ANDROID-001; iOS/WhatsApp, release distribution and background push remain deferred.
- **Verification:** See ANDROID-001 and status for exact native/offline evidence; physical-phone feedback and connected deployment acceptance remain open.
...
- **Evidence / blockers:** android/, apps/web/src/offline/, dedicated offline build/tests and android.md implement the user-authorized Android evaluation child. Parent channel/provider acceptance is still required for remaining channels.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Implementation:** In progress
- **Verification:** Current bounded-child execution is recorded under SOURCES-002; broader parent acceptance remains open.
- **Dependencies:** DEV-003, DEV-004, DEV-005; Yes
...
- **Evidence / blockers:** SOURCES-002 adds exact World Bank India unemployment/trade/FDI annual context and attributed PIB economic releases, with migration015 and source/browser/offline cases. Existing GDP/CPI canonical history remains. RBI permission and unverified MoSPI endpoints are explicit gates. Release calendars and broader macro families remain open.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Implementation:** In progress; bounded source pack implemented
- **Verification:** Current bounded-child execution is recorded under SOURCES-002; broader parent acceptance remains open.
- **Dependencies:** DEV-003, DEV-004, DEV-005; Yes
...
- **Evidence / blockers:** SOURCES-002 delivers official Fed/ECB/PIB economic releases with provenance and immutable corrections through Scan/Stories/reader/source/learning workflows. Indian company filings, entity linkage and comprehensive company news remain open; no fictional coverage is claimed.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
