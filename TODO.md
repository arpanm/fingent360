...
## Ad hoc request coverage — DELIVERY-RECONCILE-002

[Session request register](docs/development/session-request-register.md) maps all visible user requests and repeated defect reports to stable tasks, implementation evidence and remaining scope. It does not assert that inaccessible shared-chat content or unprovided attachments were read. The register is an index; this file remains the task/prompt/status authority.
...
- **Status/dependencies:** Implemented using current macro/account APIs plus protected operations sessions; integration verification below. Parents DEV-006/012/015/017.
...
- Implementation: **Planned**, **In progress** (active coding), **Partial** (delivered scope plus named pending gaps), or **Implemented** (all implementation acceptance written). **Gated**, **Deferred**, **Blocked** and **Cancelled** require an explicit reason.
...
- Implemented means the task’s required code/docs/cases are present. It does not mean validation passed, the change was committed or the task was accepted. Track verification and commit separately; user-run pnpm sdlc owns the format/check gate and commit.
...
| SDLC-002    | [User acceptance of the SDLC tool](#sdlc-002)                        | Implemented    | Awaiting user                           |
| DEV-001     | [Screen-level PRD, glossary and canonical data dictionary](#dev-001) | Implemented    | Awaiting user verification              |
| DEV-002     | [Research/advice policy and threat model](#dev-002)                  | Implemented    | Awaiting user verification              |
| DEV-003     | [Versioned domain contracts and golden fixtures](#dev-003)           | Implemented    | Awaiting user verification              |
| DEV-004     | [PostgreSQL migrations and MongoDB indexes](#dev-004)                | Implemented    | Local activation confirmed; E2E pending |
| DEV-005     | [P0 source registry and initial adapters](#dev-005)                  | Implemented    | Awaiting user verification              |
| DEV-006     | [Public intelligence slice](#dev-006)                                | Partial        | Child evidence; parent incomplete       |
| DEV-007     | [Identity/consent and manual/virtual portfolios](#dev-007)           | Implemented    | Awaiting user verification              |
| DEV-008     | [CSV/XLSX imports and help](#dev-008)                                | Partial        | Child evidence; parent incomplete       |
| DEV-009     | [Multiple goals and portfolio linkage](#dev-009)                     | Partial        | Child evidence; parent incomplete       |
| DEV-010     | [Oil-shock educational end-to-end slice](#dev-010)                   | Partial        | Child evidence; parent incomplete       |
| DEV-011     | [Daily/weekly reports and durable workers](#dev-011)                 | Partial        | Child evidence; parent incomplete       |
| DEV-012     | [Responsive PWA and accessible UI](#dev-012)                         | Implemented    | Awaiting user verification              |
| DEV-013     | [Regulated personalised advice](#dev-013)                            | Gated          | Not run                                 |
| DEV-014     | [Broker connectivity and later channels/assets](#dev-014)            | Partial        | Child evidence; parent incomplete       |
| DEV-015     | [Admin and research operations](#dev-015)                            | Partial        | Child evidence; parent incomplete       |
| DEV-016     | [Evidence, explanations and corrections](#dev-016)                   | Partial        | Child evidence; parent incomplete       |
| DEV-017     | [Privacy, security and consent lifecycle](#dev-017)                  | Partial        | Child evidence; parent incomplete       |
| DEV-018     | [Watchlists, material alerts and delivery controls](#dev-018)        | Partial        | Child evidence; parent incomplete       |
...
| DEV-021     | [Operational quality, observability and release controls](#dev-021)  | Partial        | Child evidence; parent incomplete       |
...
| DEV-029     | [WhatsApp and mobile application shells](#dev-029)                   | Partial        | Child evidence; parent incomplete       |
| DEV-030     | [Monetisation decision and commercial conflict controls](#dev-030)   | Deferred       | Not run                                 |
| SRC-001     | [Instrument/security master source onboarding (P0)](#src-001)        | Partial        | Child evidence; parent incomplete       |
...
| SRC-007     | [India macro source onboarding (P0)](#src-007)                       | Partial        | Child evidence; parent incomplete       |
| SRC-008     | [Global macro/rates source onboarding (P0)](#src-008)                | Partial        | Child evidence; parent incomplete       |
...
| SRC-012     | [Market/company news source onboarding (P0)](#src-012)               | Partial        | Child evidence; parent incomplete       |
| SRC-013     | [Portfolio spreadsheet imports source onboarding (P0)](#src-013)     | Partial        | Child evidence; parent incomplete       |
...
- **Evidence / gaps:** Vocabulary/advice gates and current account/upload/provider/operator/offline threat model authored with control links. Remaining: User design/security review. Unimplemented protections remain explicitly in DEV017/021. Evidence: docs/product/threat-model.md.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Evidence / gaps:** Real reviewed releases, annual macro, evidence/history/corrections and unavailable states. Remaining: Verified multi-point event → sector → company detail workflow and accepted data/entity links. Ordinary engineering remains, not a blanket external block. Evidence: apps/web/src/Discovery.tsx.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Evidence / gaps:** Repeated goals, editable contribution/horizon assumptions, saved comparisons and owned quantity/cost allocations. Remaining: Define and implement the stated risk-feasibility behavior without inventing returns/probabilities. Contribution-only calculations are not that policy. Evidence: packages/contracts/src/goal-scenarios.ts.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Evidence / gaps:** Durable report jobs/retries/leases, issued receipts, schedules and worker controls. Remaining: Freshness-aware research/material-change notification policy over accepted evidence; current saved-record reports and reminders do not complete it. Evidence: docs/product/report-schedules.md.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Evidence / gaps:** Umbrella task: Android shell/offline child exists. Remaining: Roll up DEV022–029; broker/channel/asset children remain. Do not duplicate their implementation or call Android absent. Evidence: docs/development/android.md.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Current delivery:** Source registry and publishing, identity refresh, cleanup, worker health, protected audit and source-comparison workflows have bounded child implementations/evidence. BEA recovery is integrated but awaiting user validation. Named roles, four-eyes approval, policy/causal review and broader operations remain open.
- **E2E cases:** Planned scenarios in tests/e2e/plans/product-coverage.md#dev-015; add stable executable IDs when implemented
- **Evidence / gaps:** Source registry, publishing/comparison, BEA recovery, identity refresh, cleanup, worker health, audit and feedback intake. Remaining: Publishing queue implemented; entity-resolution adjudication, event merge/split, causal/policy review, named roles and four-eyes approval still need implementation and defined policy. Evidence: apps/web/src/Operations.tsx.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Current delivery:** Immutable source history/withdrawal, owned evidence connections/review inbox, issued report receipts/comparison and exact publication comparison are implemented in bounded children. Broader causal explanations, equity valuation and recommendation reconstruction remain open.
- **E2E cases:** Planned scenarios in tests/e2e/plans/product-coverage.md#dev-016; add stable executable IDs when implemented
- **Evidence / gaps:** Reader/source/history, glossary, own evidence links/review inbox, immutable report receipts and source correction/withdrawal. Remaining: Real entailed one-line → beginner → portfolio → analytical → source layers with precise claim/source/time/conflict semantics. Valuation is not an added completion criterion. Evidence: apps/api/src/research-connections.ts.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Evidence / gaps:** Ownership/consent, recovery/session revoke, complete export/deletion, cleanup, isolated parsing and final read admission. Remaining: Granular consent purpose/expiry lifecycle, PII encryption/key handling, support-access roles/audit and remaining injection/security controls. Broker vault only when broker scope is selected. Evidence: docs/product/threat-model.md.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Current delivery:** Watchlists, private reading reminders and explicit source/topic reading-update baselines/checks/acknowledgement/history are implemented in bounded children. Material-event policy, continuous monitoring and external email/push delivery remain open.
- **E2E cases:** Planned scenarios in tests/e2e/plans/product-coverage.md#dev-018; add stable executable IDs when implemented
- **Evidence / gaps:** Watchlists, observation receipts/mutes, reading reminders and explicit follow/coalescing/ack/history. Remaining: Release/calendar context and defined materiality thresholds/batching with unchanged/nonmaterial/no-urgency acceptance. External email/push is not automatically required. Evidence: apps/api/src/reading-follow.ts.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
...
- **Evidence / gaps:** Fed/ECB and BEA released headlines/evidence/review/withdrawal, plus authored BEA recovery. Remaining: Choose minimum numerical India-context macro/rate series, implement accepted numerical/vintage adapters and limits/reconciliation; not every candidate provider is mandatory. Evidence: docs/product/bea-releases.md.
- **Manual next actions:** After implementation, user runs relevant checks and selected E2E cases listed in the handoff; attach evidence before marking verified.
