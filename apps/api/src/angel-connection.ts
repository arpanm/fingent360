...
const ANGEL = Symbol('ANGEL_CONNECTION'),
...
function status(row: Record<string, unknown> | undefined, enabled: boolean) {
  const state = !row
    ...
    : (row.state === 'connected' &&
      ...
      ? 'expired'
      : row.state;
  return AngelConnectionStatusSchema.parse({
    enabled,
...
export async function exportAngelConnections(
  c: pg.PoolClient,
  userId: string,
  ...
  const connection = (
      await c.query(
        'SELECT state,pending_until,connected_at,expires_at,last_fetched_at FROM broker_angel_connections WHERE user_id=$1',
        [userId],
      )
    ).rows[0],
    rows = (
      await c.query(
        'SELECT * FROM broker_angel_captures WHERE user_id=$1 ORDER BY retrieved_at,id',
        [userId],
      )
    ).rows,
    captures = [];
  for (const row of rows) {
    ...
    if (privatePayloadNeedsRotation(row.encrypted_payload, keys))
      await c.query(
        'UPDATE broker_angel_captures SET encrypted_payload=$3 WHERE user_id=$1 AND id=$2',
        [
          userId,
          row.id,
          sealPrivateJson(
  ...
  const events = (
    await c.query(
      'SELECT action,recorded_at FROM broker_angel_events WHERE user_id=$1 ORDER BY recorded_at,id',
      [userId],
    )
  ).rows;
  return AngelExportSchema.parse({
...
export class AngelConnectionStore {
  constructor(
    private readonly account: AccountStore,
    private readonly config: AppConfig,
    private readonly provider = new AngelProvider(
      config.ANGEL_API_KEY || '',
      config.ANGEL_CLIENT_LOCAL_IP || '',
      config.ANGEL_CLIENT_PUBLIC_IP || '',
      config.ANGEL_MAC_ADDRESS || '',
    ),
  ) {}
  enabled() {
    return !!(
  ...
  private ready() {
    ...
    const redirect = new URL(this.config.ANGEL_REDIRECT_URL!);
    if (
      redirect.pathname !==
        '/api/v1/account/broker-connections/angel/callback' ||
      redirect.search ||
      redirect.hash ||
      redirect.username ||
      redirect.password ||
  ...
  async cancel(raw: unknown, cookie?: string) {
    ...
    await this.account.transaction(async (c) => {
      ...
      const result = await c.query(
        "UPDATE broker_angel_connections SET state='revoked',state_hash=NULL,pending_until=NULL,version=version+1 WHERE user_id=$1 AND state_hash=$2 AND state='pending'",
      ...
      await c.query(
        'INSERT INTO broker_angel_events(id,user_id,action) VALUES($1,$2,$3)',
  ...
  callbackOrigin(origin?: string) {
    if (
      ...
      ![
        ...
        new URL(this.config.ANGEL_REDIRECT_URL || this.config.WEB_ORIGIN)
          .origin,
      ].includes(origin)
    )
  ...
  async read(cookie?: string) {
    return this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      await c.query(
        "UPDATE broker_angel_connections SET state='expired',encrypted_token=NULL,state_hash=NULL WHERE user_id=$1 AND ((state='connected' AND expires_at<=clock_timestamp()) OR (state='pending' AND pending_until<=clock_timestamp()))",
        [user.id],
      );
      return status(
        (
  ...
  async start(raw: unknown, cookie?: string) {
    ...
    await this.account.transaction(async (c) => {
      ...
      const previous = (
        await c.query(
          'SELECT state,expires_at FROM broker_angel_connections WHERE user_id=$1',
          [user.id],
        )
      ).rows[0];
      if (
      ...
      const attempts = await c.query(
        "SELECT count(*)::int AS count FROM broker_angel_events WHERE user_id=$1 AND action='connection-consent' AND recorded_at>now()-interval '1 day'",
        [user.id],
      );
      ...
      await c.query(
        "INSERT INTO broker_angel_connections(user_id,state,state_hash,pending_until,permission_reference) VALUES($1,'pending',$2,$3,$4) ON CONFLICT(user_id) DO UPDATE SET state='pending',version=broker_angel_connections.version+1,state_hash=$2,pending_until=$3,encrypted_token=NULL,expires_at=NULL,permission_reference=$4",
        [user.id, hash(nonce), until, this.config.ANGEL_PERMISSION_REFERENCE],
      );
      await c.query(
        'INSERT INTO broker_angel_events(id,user_id,action) VALUES($1,$2,$3)',
    ...
    url.search = new URLSearchParams({
      api_key: this.config.ANGEL_API_KEY!,
      redirect_url: this.config.ANGEL_REDIRECT_URL!,
      state: nonce,
    }).toString();
    return AngelStartResultSchema.parse({
      loginUrl: url.href,
      expiresAt: until,
    });
  }
  async callback(query: unknown, cookie?: string) {
    ...
    const value = input(
      z.strictObject({
        auth_token: z.string().regex(/^[A-Za-z0-9._~-]{8,4096}$/),
    ...
    const claim = await this.account.transaction(async (c) => {
      ...
      const row = (
        await c.query(
          "UPDATE broker_angel_connections SET state_hash=NULL WHERE state_hash=$1 AND user_id=$2 AND state='pending' AND pending_until>clock_timestamp() RETURNING user_id,version",
    ...
    let vault;
    try {
      vault = await this.provider.exchange(value.auth_token);
    } catch {
      throw new ServiceUnavailableException(
    ...
    try {
      await this.account.transaction(async (c) => {
        ...
        const result = await c.query(
          "UPDATE broker_angel_connections SET state='connected',encrypted_token=$3,connected_at=clock_timestamp(),expires_at=$4,pending_until=NULL WHERE user_id=$1 AND version=$2 AND state='pending' AND pending_until>clock_timestamp()",
          [
            claim.user_id,
            claim.version,
            sealPrivateJson(
  ...
  async fetch(raw: unknown, cookie?: string) {
    const value = input(AngelFetchSchema, raw);
    this.ready();
    const claim = await this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      ...
      const row = (
        await c.query(
          "SELECT * FROM broker_angel_connections WHERE user_id=$1 AND state='connected' AND expires_at>clock_timestamp() FOR UPDATE",
          [user.id],
        )
      ).rows[0];
      if (!row)
      ...
      const vault = AngelVaultSchema.parse(
        openPrivateJson(
          'broker-angel-token',
          user.id,
          user.id,
      ...
      if (
        ...
        await c.query(
          'UPDATE broker_angel_connections SET encrypted_token=$3 WHERE user_id=$1 AND version=$2',
          [
            user.id,
            row.version,
            sealPrivateJson(
    ...
    let holdings;
    try {
      holdings = parseAngelHoldings(rawResponse);
    } catch {
      throw new BadRequestException(
    ...
    const id = randomUUID(),
      capture = AngelCaptureSchema.parse({
        id,
        provider: 'angel',
        parserVersion: 'angel-settled-holdings-v1',
    ...
    const preview = await new HoldingsController(this.account).createPreview(
      ...
      async (c, userId, previewId) => {
        const current = await c.query(
          "UPDATE broker_angel_connections SET last_fetched_at=clock_timestamp() WHERE user_id=$1 AND version=$2 AND state='connected' AND expires_at>clock_timestamp() RETURNING user_id",
        ...
        const count = await c.query(
          'SELECT count(*)::int AS count FROM broker_angel_captures WHERE user_id=$1',
          [userId],
        );
        ...
        await c.query(
          'INSERT INTO broker_angel_captures(id,user_id,preview_id,encrypted_payload,source_hash,retrieved_at) VALUES($1,$2,$3,$4,$5,$6)',
          [
            id,
            userId,
            previewId,
    ...
    return AngelPreviewResultSchema.parse({ capture, preview });
  }
  async revoke(raw: unknown, cookie?: string) {
    input(AngelRevokeSchema, raw);
    let vault: z.infer<typeof AngelVaultSchema> | null = null;
    await this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      await c.query('SELECT id FROM app_users WHERE id=$1 FOR UPDATE', [
        user.id,
      ...
      const row = (
        await c.query(
          'SELECT state,encrypted_token FROM broker_angel_connections WHERE user_id=$1 FOR UPDATE',
          [user.id],
        )
      ).rows[0];
      ...
      await c.query(
        "UPDATE broker_angel_connections SET state='revoked',version=version+1,encrypted_token=NULL,state_hash=NULL,pending_until=NULL WHERE user_id=$1",
        [user.id],
      );
      await c.query(
        'DELETE FROM app_holdings_previews WHERE user_id=$1 AND confirmed_version IS NULL AND id IN (SELECT preview_id FROM broker_angel_captures WHERE user_id=$1)',
        [user.id],
      );
      await c.query(
        'INSERT INTO broker_angel_events(id,user_id,action) VALUES($1,$2,$3)',
    ...
    return AngelRevokedSchema.parse({
      revoked: true,
      remote,
      message:
        remote === 'confirmed'
          ? 'Connection revoked. Saved confirmed holdings remain.'
          : 'Local connection revoked. Use Angel logout to invalidate any remaining broker session; saved confirmed holdings remain.',
    });
  }
  async export(cookie?: string) {
    return this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      return exportAngelConnections(
        c,
        user.id,
        this.account.privateDataKeys,
        this.enabled(),
  ...
  async remove(raw: unknown, cookie?: string) {
    ...
    return this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      await c.query('DELETE FROM broker_angel_captures WHERE user_id=$1', [
        user.id,
      ]);
      await c.query('DELETE FROM broker_angel_events WHERE user_id=$1', [
        user.id,
      ]);
      await c.query('DELETE FROM broker_angel_connections WHERE user_id=$1', [
        user.id,
      ]);
      return { deleted: true };
    });
...
@Controller('account/broker-connections/angel')
export class AngelConnectionController {
  constructor(
    @Inject(ANGEL) private readonly store: AngelConnectionStore,
  ...
  @Get('callback') callback(
    ...
    const parsed = z
      .strictObject({
        auth_token: z.string().regex(/^[A-Za-z0-9._~-]{8,4096}$/),
        feed_token: z.string().max(4096).optional(),
        state: z.string().regex(/^[a-f0-9]{64}$/),
      })
      .safeParse(query);
    res.setHeader('Cache-Control', 'no-store');
...
export function angelConnectionProvider(config: AppConfig) {
  return {
    provide: ANGEL,
    inject: [STORE],
    useFactory: (account: AccountStore) =>
      new AngelConnectionStore(account, config),
  };
}
