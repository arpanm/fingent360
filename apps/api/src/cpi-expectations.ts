...
export const cpiExpectationProvider = (config: AppConfig) => ({
  provide: RAW,
  useValue: new CpiExpectationRaw(config),
});
...
@Controller('cpi-expectations')
export class CpiExpectationsController {
  constructor(@Inject(STORE) private readonly store: AccountStore) {}
  @Get() read() {
    return this.store.transaction(async (c) => {
      await c.query('LOCK TABLE cpi_expectation_reviews IN SHARE MODE');
      const rows = await c.query(
        "SELECT e.payload,(SELECT created_at FROM cpi_expectation_reviews WHERE edition_id=e.id ORDER BY seq DESC LIMIT 1) AS reviewed_at FROM cpi_expectation_editions e WHERE (SELECT decision FROM cpi_expectation_reviews WHERE edition_id=e.id ORDER BY seq DESC LIMIT 1)='publish' ORDER BY e.created_at DESC,e.id LIMIT 101",
      );
      if (rows.rows.length > 100)
        throw new ConflictException(
          'CPI snapshot limit reached. Withdraw obsolete editions before publishing more.',
        );
      const response = CpiExpectationPublicSchema.parse({
        capturedAt: new Date().toISOString(),
        editions: rows.rows.map((row) => {
          const edition = CpiExpectationEditionSchema.parse(row.payload);
          return {
            id: edition.id,
      ...
      const comparisons = response.editions
        .filter((e) => e.expectation.kind !== 'observed-release')
        .flatMap((n) =>
          n.expectation.points.flatMap((p) =>
            response.editions
              .filter(
                (a) =>
                  a.expectation.kind === 'observed-release' &&
                  a.expectation.points.some(
                    (point) => point.period === p.period,
                  ),
              )
              .map((a) => ({
                nowcastId: n.id,
                actualId: a.id,
                period: p.period,
                result: compareCpiSnapshots(n, a, p.period),
              })),
          ),
        );
      const view = { editions: response.editions, comparisons };
      await c.query(
        'INSERT INTO cpi_expectation_views(hash,payload) VALUES($1,$2) ON CONFLICT DO NOTHING',
        [digest(view), view],
      );
      return response;
    });
...
@Controller('ops/cpi-expectations')
export class CpiExpectationOperationsController {
  constructor(
    @Inject(STORE) private readonly store: AccountStore,
    @Inject(OPERATOR_STORE) private readonly ops: OperatorStore,
    @Inject(RAW) private readonly raw: CpiExpectationRaw,
  ) {}
  private async actor(
  ...
  @OperatorRead() @Get() queue(
    ...
    return this.store.transaction(async (c) => {
      ...
      const rows = await c.query(
        `SELECT id,payload,to_char(created_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS page_at,COALESCE((SELECT CASE decision WHEN 'publish' THEN 'published' ELSE 'withdrawn' END FROM cpi_expectation_reviews WHERE edition_id=e.id ORDER BY seq DESC LIMIT 1),'draft') AS state FROM cpi_expectation_editions e WHERE ($1::timestamptz IS NULL OR (created_at,id)<($1::timestamptz,$2::uuid)) ORDER BY created_at DESC,id DESC LIMIT 26`,
        [parts?.[0] ?? null, parts?.[1] ?? null],
      );
      const page = rows.rows.slice(0, 25),
        result = CpiExpectationQueueSchema.parse({
  ...
  @OperatorRead() @Get(':id/evidence') async evidence(
    ...
    const edition = await this.store.transaction(async (c) => {
      const row = await c.query(
        'SELECT payload FROM cpi_expectation_editions WHERE id=$1',
        [id],
      );
      await this.actor(cookie, 'read', c);
  ...
  @OperatorAction('prepare') @Post('import') async capture(
    ...
    return this.retain(input(CpiExpectationInputSchema, body), cookie);
  }
  @OperatorAction('prepare') @Post('fetch') async fetchReport(
    ...
    const data = input(CpiExpectationInputSchema.omit({ body: true }), body);
    ...
    if (old) {
      const edition = CpiExpectationEditionSchema.parse(old);
      if (
        edition.expectation.url !== data.url ||
        edition.rightsBasis !== data.rightsBasis ||
        digest(
          edition.expectation.history
            ? {
                period: edition.expectation.history.period,
                asOf: edition.expectation.history.asOf,
              }
            : null,
        ) !== digest(data.historySelection ?? null)
      )
    ...
    if (!response.ok || !response.body)
      throw new ServiceUnavailableException('Original CPI source unavailable.');
    ...
    try {
      for (;;) {
        ...
        if (size > 10000000)
          throw new BadRequestException('CPI source exceeds 10 MB.');
  ...
  private async retain(
    data: z.infer<typeof CpiExpectationInputSchema>,
    cookie?: string,
  ) {
    ...
    try {
      expectation = parseCpiCapture({
        ...
        hash,
        retrievedAt,
        historySelection: data.historySelection,
      });
    } catch {
      throw new BadRequestException(
        'Retained report does not match the verified Cleveland/BLS monthly CPI layout.',
      );
    }
    const edition = CpiExpectationEditionSchema.parse({
        id: data.requestId,
        expectation,
    ...
    return this.store.transaction(async (c) => {
      ...
      const old = await c.query(
        'SELECT fingerprint,payload FROM cpi_expectation_editions WHERE id=$1',
        [data.requestId],
      );
      await this.actor(cookie, 'prepare', c);
      if (old.rows[0]) {
        ...
        return CpiExpectationEditionSchema.parse(old.rows[0].payload);
      }
      await c.query(
        'INSERT INTO cpi_expectation_editions(id,fingerprint,payload) VALUES($1,$2,$3)',
  ...
  @OperatorAction('approve') @Post('review') review(
    ...
    this.ops.origin(origin);
    const data = input(CpiExpectationReviewSchema, body);
    return this.store.transaction(async (c) => {
      ...
      const old = await c.query(
        'SELECT edition_id,decision,reason FROM cpi_expectation_reviews WHERE request_id=$1',
        [data.requestId],
      );
      if (old.rows[0]) {
      ...
      const rows = await c.query(
        'SELECT payload FROM cpi_expectation_editions WHERE id=$1 FOR UPDATE',
        [data.editionId],
      );
      if (!rows.rows[0])
        throw new NotFoundException('Expectation unavailable.');
      const edition = CpiExpectationEditionSchema.parse(rows.rows[0].payload);
        ...
        if (
          ...
          digest(
            parseCpiCapture({
              ...
              historySelection: edition.expectation.history
                ? {
                    period: edition.expectation.history.period,
                    asOf: edition.expectation.history.asOf,
                  }
      ...
      await c.query(
        'INSERT INTO cpi_expectation_reviews(request_id,edition_id,actor_id,decision,reason) VALUES($1,$2,$3,$4,$5)',
