...
function owner(authorization?: string): string {
  if (!authorization || !/^Bearer [a-f0-9]{64}$/.test(authorization))
    throw new UnauthorizedException('Open a virtual workspace first.');
...
export class JourneyStore {
  ...
  constructor(config: AppConfig) {
    ...
    this.pool.on('error', () => {
      /* Requests report availability without logging connection details. */
    });
  }
  ...
  async read(key: string) {
    return this.transaction((c) => this.locked(c, key));
  }
  async save(key: string, raw: unknown) {
    ...
    return this.mutate(
      ...
      async () => ({ portfolio: input.portfolio }),
    );
  }
  async preview(key: string, raw: unknown) {
    ...
    await this.transaction(async (c) => {
      ...
      await c.query(
        "DELETE FROM virtual_previews WHERE owner_hash=$1 AND created_at < now() - interval '1 day'",
        [key],
      );
      const count = await c.query(
        'SELECT count(*)::int AS count FROM virtual_previews WHERE owner_hash=$1',
        [key],
      );
  ...
  private async mutate(
    ...
    resolve: (
      c: pg.PoolClient,
      current: z.infer<typeof WorkspaceSchema>,
    ) => Promise<{ portfolio: PortfolioInput; contentHash?: string }>,
  ) {
...
export class JourneyController {
  ...
  @Get('workspace') read(@Headers('authorization') auth?: string) {
    return this.store.read(owner(auth));
  }
  @Post('workspace') @HttpCode(200) save(
    @Body() body: unknown,
    @Headers('authorization') auth?: string,
  ) {
    return this.store.save(owner(auth), body);
  }
  @Delete('workspace') remove(@Headers('authorization') auth?: string) {
    return this.store.remove(owner(auth));
  }
  @Post('previews') preview(
    @Body() body: unknown,
    @Headers('authorization') auth?: string,
  ) {
    return this.store.preview(owner(auth), body);
  }
  @Post('imports') @HttpCode(200) confirm(
    @Body() body: unknown,
    @Headers('authorization') auth?: string,
  ) {
    return this.store.confirm(owner(auth), body);
  }
  @Post('reviews') review(
    @Body() body: unknown,
    @Headers('authorization') auth?: string,
  ) {
    return this.store.review(owner(auth), body);
  }
  @Get('reviews') reviews(@Headers('authorization') auth?: string) {
    return this.store.reviews(owner(auth));
  }
  @Get('reviews/:id') detail(
    @Param('id') id: string,
    @Headers('authorization') auth?: string,
  ) {
    return this.store.reviewDetail(owner(auth), id);
  }
}
export function journeyProvider(config: AppConfig) {
  return { provide: STORE, useValue: new JourneyStore(config) };
}
