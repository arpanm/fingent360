...
const KITE = Symbol('KITE_CONNECTION'),
...
function status(row: Record<string, unknown> | undefined, enabled: boolean) {
  const state = !row
    ...
    : (row.state === 'connected' &&
      ...
      ? 'expired'
      : row.state;
  return KiteConnectionStatusSchema.parse({
    enabled,
...
export async function exportKiteConnections(
  c: pg.PoolClient,
  userId: string,
  ...
  const connection = (
      await c.query(
        'SELECT state,pending_until,connected_at,expires_at,last_fetched_at FROM broker_kite_connections WHERE user_id=$1',
        [userId],
      )
    ).rows[0],
    rows = (
      await c.query(
        'SELECT * FROM broker_kite_captures WHERE user_id=$1 ORDER BY retrieved_at,id',
        [userId],
      )
    ).rows,
    captures = [];
  for (const row of rows) {
    ...
    if (privatePayloadNeedsRotation(row.encrypted_payload, keys))
      await c.query(
        'UPDATE broker_kite_captures SET encrypted_payload=$3 WHERE user_id=$1 AND id=$2',
        [
          userId,
          row.id,
          sealPrivateJson('broker-kite-capture', userId, row.id, decoded, keys),
        ],
      );
  ...
  const events = (
    await c.query(
      'SELECT action,recorded_at FROM broker_kite_events WHERE user_id=$1 ORDER BY recorded_at,id',
      [userId],
    )
  ).rows;
  return KiteExportSchema.parse({
...
export class KiteConnectionStore {
  constructor(
    private readonly account: AccountStore,
    private readonly config: AppConfig,
    private readonly provider = new KiteProvider(
      config.KITE_API_KEY || '',
  ...
  private ready() {
    ...
    const redirect = new URL(this.config.KITE_REDIRECT_URL!);
    if (
      redirect.pathname !==
        '/api/v1/account/broker-connections/kite/callback' ||
      redirect.search ||
      redirect.hash ||
      redirect.username ||
      redirect.password ||
  ...
  async cancel(raw: unknown, cookie?: string) {
    ...
    await this.account.transaction(async (c) => {
      ...
      const result = await c.query(
        "UPDATE broker_kite_connections SET state='revoked',state_hash=NULL,pending_until=NULL,version=version+1 WHERE user_id=$1 AND state_hash=$2 AND state='pending'",
      ...
      await c.query(
        'INSERT INTO broker_kite_events(id,user_id,action) VALUES($1,$2,$3)',
  ...
  callbackOrigin(origin?: string) {
    if (
      ...
      ![
        ...
        new URL(this.config.KITE_REDIRECT_URL || this.config.WEB_ORIGIN).origin,
      ].includes(origin)
    )
  ...
  async read(cookie?: string) {
    return this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      await c.query(
        "UPDATE broker_kite_connections SET state='expired',encrypted_token=NULL,state_hash=NULL WHERE user_id=$1 AND ((state='connected' AND expires_at<=clock_timestamp()) OR (state='pending' AND pending_until<=clock_timestamp()))",
        [user.id],
      );
      return status(
        (
  ...
  async start(raw: unknown, cookie?: string) {
    ...
    await this.account.transaction(async (c) => {
      ...
      const previous = (
        await c.query(
          'SELECT state,expires_at FROM broker_kite_connections WHERE user_id=$1',
          [user.id],
        )
      ).rows[0];
      if (
      ...
      const attempts = await c.query(
        "SELECT count(*)::int AS count FROM broker_kite_events WHERE user_id=$1 AND action='connection-consent' AND recorded_at>now()-interval '1 day'",
        [user.id],
      );
      ...
      await c.query(
        "INSERT INTO broker_kite_connections(user_id,state,state_hash,pending_until,permission_reference) VALUES($1,'pending',$2,$3,$4) ON CONFLICT(user_id) DO UPDATE SET state='pending',version=broker_kite_connections.version+1,state_hash=$2,pending_until=$3,encrypted_token=NULL,expires_at=NULL,permission_reference=$4",
        [user.id, hash(nonce), until, this.config.KITE_PERMISSION_REFERENCE],
      );
      await c.query(
        'INSERT INTO broker_kite_events(id,user_id,action) VALUES($1,$2,$3)',
    ...
    url.search = new URLSearchParams({
      v: '3',
      api_key: this.config.KITE_API_KEY!,
      redirect_params: new URLSearchParams({ state: nonce }).toString(),
    }).toString();
    return KiteStartResultSchema.parse({
      loginUrl: url.href,
      expiresAt: until,
    });
  }
  async callback(query: unknown, cookie?: string) {
    ...
    const value = input(
      z.strictObject({
        request_token: z.string().regex(/^[A-Za-z0-9_-]{8,256}$/),
    ...
    const claim = await this.account.transaction(async (c) => {
      ...
      const row = (
        await c.query(
          "UPDATE broker_kite_connections SET state_hash=NULL WHERE state_hash=$1 AND user_id=$2 AND state='pending' AND pending_until>clock_timestamp() RETURNING user_id,version",
    ...
    let vault;
    try {
      vault = await this.provider.exchange(value.request_token);
    } catch {
      throw new ServiceUnavailableException(
    ...
    try {
      await this.account.transaction(async (c) => {
        ...
        const result = await c.query(
          "UPDATE broker_kite_connections SET state='connected',encrypted_token=$3,connected_at=clock_timestamp(),expires_at=$4,pending_until=NULL WHERE user_id=$1 AND version=$2 AND state='pending' AND pending_until>clock_timestamp()",
          [
            claim.user_id,
            claim.version,
            sealPrivateJson(
  ...
  async fetch(raw: unknown, cookie?: string) {
    const value = input(KiteFetchSchema, raw);
    this.ready();
    const claim = await this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      ...
      const row = (
        await c.query(
          "SELECT * FROM broker_kite_connections WHERE user_id=$1 AND state='connected' AND expires_at>clock_timestamp() FOR UPDATE",
          [user.id],
        )
      ).rows[0];
      if (!row)
      ...
      const vault = KiteVaultSchema.parse(
        openPrivateJson(
          'broker-kite-token',
          user.id,
          user.id,
      ...
      if (
        ...
        await c.query(
          'UPDATE broker_kite_connections SET encrypted_token=$3 WHERE user_id=$1 AND version=$2',
          [
            user.id,
            row.version,
            sealPrivateJson(
    ...
    let holdings;
    try {
      holdings = parseKiteHoldings(rawResponse);
    } catch {
      throw new BadRequestException(
    ...
    const id = randomUUID(),
      capture = KiteCaptureSchema.parse({
        id,
        provider: 'zerodha-kite',
        parserVersion: 'kite-settled-holdings-v1',
        retrievedAt: new Date().toISOString(),
        sourceUrl: 'https://api.kite.trade/portfolio/holdings',
    ...
    const preview = await new HoldingsController(this.account).createPreview(
      ...
      async (c, userId, previewId) => {
        const current = await c.query(
          "UPDATE broker_kite_connections SET last_fetched_at=clock_timestamp() WHERE user_id=$1 AND version=$2 AND state='connected' AND expires_at>clock_timestamp() RETURNING user_id",
        ...
        const count = await c.query(
          'SELECT count(*)::int AS count FROM broker_kite_captures WHERE user_id=$1',
          [userId],
        );
        ...
        await c.query(
          'INSERT INTO broker_kite_captures(id,user_id,preview_id,encrypted_payload,source_hash,retrieved_at) VALUES($1,$2,$3,$4,$5,$6)',
          [
            id,
            userId,
            previewId,
    ...
    return KitePreviewResultSchema.parse({ capture, preview });
  }
  async revoke(raw: unknown, cookie?: string) {
    input(KiteRevokeSchema, raw);
    let vault: z.infer<typeof KiteVaultSchema> | null = null;
    await this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      await c.query('SELECT id FROM app_users WHERE id=$1 FOR UPDATE', [
        user.id,
      ...
      const row = (
        await c.query(
          'SELECT state,encrypted_token FROM broker_kite_connections WHERE user_id=$1 FOR UPDATE',
          [user.id],
        )
      ).rows[0];
      ...
      await c.query(
        "UPDATE broker_kite_connections SET state='revoked',version=version+1,encrypted_token=NULL,state_hash=NULL,pending_until=NULL WHERE user_id=$1",
        [user.id],
      );
      await c.query(
        'DELETE FROM app_holdings_previews WHERE user_id=$1 AND confirmed_version IS NULL AND id IN (SELECT preview_id FROM broker_kite_captures WHERE user_id=$1)',
        [user.id],
      );
      await c.query(
        'INSERT INTO broker_kite_events(id,user_id,action) VALUES($1,$2,$3)',
    ...
    return KiteRevokedSchema.parse({
      revoked: true,
      remote,
      message:
        remote === 'confirmed'
          ? 'Connection revoked. Saved confirmed holdings remain.'
          : 'Local connection revoked. Use Kite logout to invalidate any remaining broker session; saved confirmed holdings remain.',
    });
  }
  async export(cookie?: string) {
    return this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      return exportKiteConnections(
        c,
        user.id,
        this.account.privateDataKeys,
        this.enabled(),
  ...
  async remove(raw: unknown, cookie?: string) {
    ...
    return this.account.transaction(async (c) => {
      const user = await this.account.require(c, cookie);
      await c.query('DELETE FROM broker_kite_captures WHERE user_id=$1', [
        user.id,
      ]);
      await c.query('DELETE FROM broker_kite_events WHERE user_id=$1', [
        user.id,
      ]);
      await c.query('DELETE FROM broker_kite_connections WHERE user_id=$1', [
        user.id,
      ]);
      return { deleted: true };
    });
...
@Controller('account/broker-connections/kite')
export class KiteConnectionController {
  constructor(
    @Inject(KITE) private readonly store: KiteConnectionStore,
...
export function kiteConnectionProvider(config: AppConfig) {
  return {
    provide: KITE,
    inject: [STORE],
    useFactory: (account: AccountStore) =>
      new KiteConnectionStore(account, config),
  };
}
