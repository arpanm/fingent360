...
test('half-even rounding and zero denominator are explicit', () => {
  ...
  assert.equal(
    valuePortfolio({ holdings: [], cash: '0.00', goals: [] }).affectedPercent,
    null,
  );
  assert.equal(
    valuePortfolio({
      ...portfolio,
      holdings: [{ instrumentId: 'alpha-air', quantity: '0.00015' }],
    }).positions[0].value,
    '0.02',
  );
});
test('invalid financial inputs and excessive allocations reject', () => {
  for (const quantity of ['-1', '1e3', 'NaN', '0', '1.0000001'])
    assert.equal(
      PortfolioInputSchema.safeParse({
        ...portfolio,
        holdings: [{ instrumentId: 'alpha-air', quantity }],
      }).success,
      false,
    );
  assert.equal(
    PortfolioInputSchema.safeParse({
      ...portfolio,
      goals: [{ ...goal, allocationPercent: 101 }],
    }).success,
    false,
  );
  assert.equal(
    PortfolioInputSchema.safeParse({ ...portfolio, extra: true }).success,
    false,
  );
});
test('review blockers never become no-review conclusions', () => {
  ...
  for (const scenario of ['stale', 'conflicting'])
    assert.equal(
      createReview(portfolio, 1, scenario).status,
      'unable_to_assess',
    );
  assert.equal(
    createReview({ ...portfolio, goals: [] }, 1, 'baseline').status,
    'unable_to_assess',
  );
  assert.equal(
    createReview(
      {
        ...portfolio,
        holdings: [{ instrumentId: 'bharat-software', quantity: '1' }],
      },
      1,
      'baseline',
    ).status,
...
test('CSV rejects duplicate/unknown rows and accepts BOM/CRLF', () => {
  assert.deepEqual(
    parseCsv('\uFEFFinstrumentId,quantity\r\nalpha-air,10\r\n').holdings,
    portfolio.holdings,
  );
  for (const csv of [
...
test('minor-unit allocation remainder is retained and boundary sums stay valid', () => {
  const result = valuePortfolio({
    holdings: [],
    cash: '0.01',
    goals: [{ ...goal, allocationPercent: 50 }],
  });
  assert.equal(result.goals[0].funded, '0.00');
  assert.equal(result.unallocated, '0.01');
  assert.equal(
    valuePortfolio({
      holdings: [{ instrumentId: 'alpha-air', quantity: '999999999' }],
