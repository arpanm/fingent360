...
test('storage diagnostics distinguish setup failures without exposing driver data', () => {
  for (const [code, expected] of [
    ...
    const message = storageErrorMessage({ code, message: 'private-password', detail: 'private-query' });
...
test('unknown errors do not imply migration is the confirmed cause', () => {
  for (const error of [null, undefined, new Error('private-password'), { code: 'unknown' }]) {
...
test('workspace transaction preserves the schema diagnostic and rolls back', async () => {
  ...
  await assert.rejects(
    () => store.transaction(async () => { throw { code: '42P01', message: 'private SQL' }; }),
    (error) => error.getStatus() === 503 && /schema is missing/.test(error.message) && !error.message.includes('private SQL'),
  );
