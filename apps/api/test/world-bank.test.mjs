...
test('source numeric lexemes are retained exactly, including exponent and null', () => {
  assert.equal(parseWorldBank(response(),key).observations[0].value, '1.234567890123456789');
  assert.equal(parseWorldBank(response('1.234e-4'),key).observations[0].value, '0.0001234');
  assert.equal(parseWorldBank(response('null'),key).observations[0].value, null);
  assert.equal(normalizeDecimal('-0.000'), '0');
});
test('schema, identity, units, partial pages and future observations quarantine', () => {
  for (const body of [response('1',',"unexpected":true'), response().replace('"IND"','"USA"'), response().replace('"pages":1','"pages":2'), response().replace('"unit":""','"unit":"USD"'), response().replace('"2024"','"2099"'), '<html>failure</html>'])
