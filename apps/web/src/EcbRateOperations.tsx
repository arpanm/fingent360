...
import { EcbEdition } from './EcbRates';
import { useDraftGuard } from './useDraftGuard';
type Overview = ReturnType<typeof EcbRateOperationsSchema.parse>;
type Intent =
  | { kind: 'refresh'; body: { requestId: string } }
  | { kind: 'review'; body: ReturnType<typeof EcbRateReviewSchema.parse> };
export function EcbRateOperations({
  request,
  onDenied,
  onPropose,
}: {
  ...
  const [data, setData] = useState<Overview | null>(null),
    [runs, setRuns] = useState<ReturnType<
      typeof EcbRateRunsSchema.parse
    > | null>(null);
  const [ready, setReady] = useState(false),
    [busy, setBusy] = useState(true);
  const [reviews, setReviews] = useState<ReturnType<
    typeof EcbRateReviewsSchema.parse
  > | null>(null);
  ...
  const [pending, setPending] = useState<Intent | null>(null),
    [receipt, setReceipt] = useState<ReturnType<
      typeof EcbRateReviewReceiptSchema.parse
    > | null>(null);
  const [raw, setRaw] = useState<ReturnType<
      typeof EcbRateRetainedSchema.parse
    > | null>(null),
  ...
  useDraftGuard(
    !!reason || !!pending,
    'Leave and discard the unsaved policy-rate review?',
  );
    ...
    if (cause instanceof RequestError && cause.status === 401) {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : 'Policy-rate operation unavailable. Retry.',
      );
  }
  async function load() {
    ...
    try {
      const [overview, history, savedReviews] = await Promise.all([
        requestRef
          .current('/ops/policy-rates')
          .then(EcbRateOperationsSchema.parse),
        requestRef
          .current('/ops/policy-rates/runs')
          .then(EcbRateRunsSchema.parse),
        requestRef
          .current('/ops/policy-rates/reviews')
          .then(EcbRateReviewsSchema.parse),
      ]);
  ...
  async function submit(intent: Intent) {
    ...
    try {
      if (intent.kind === 'refresh') {
        const result = EcbRateRunSchema.parse(
          await requestRef.current(
            '/ops/policy-rates/refresh',
            intent.body,
            'POST',
          ),
        );
        ...
        await onPropose({
          kind: 'ecb-rates',
          target: ECB_RATE_SOURCE,
          body: intent.body,
        });
        if (!live.current || denied.current || ticket !== generation.current)
          return;
        setNotice(
          'Policy-rate proposal saved. A different named approver must review it in Named operators and proposals.',
        );
      } else {
        const result = EcbRateReviewReceiptSchema.parse(
          await requestRef.current(
            '/ops/policy-rates/review',
            intent.body,
            'PUT',
          ),
        );
  ...
  return (
    <section
      className="panel ecb-rates"
      aria-label="ECB policy-rate operations"
      data-feedback-private
    >
      <h2>ECB policy rates</h2>
      <p>
        One fixed official numerical source, three euro-area policy rates.
        Refresh captures a draft; publishing and withdrawal require an explicit
        review.
      </p>
      {error && <p role="alert">{error}</p>}
      {notice && <p role="status">{notice}</p>}
      {busy && <p role="status">Reading or saving policy-rate state…</p>}
      <button disabled={busy || !!pending} onClick={() => void load()}>
        {error ? 'Retry policy-rate state' : 'Reload policy-rate state'}
      </button>
      <button
      ...
      {pending && (
        <>
          ...
          <button disabled={busy} onClick={() => void submit(pending)}>
            Retry same policy-rate request
          </button>
          <button
            disabled={busy}
            onClick={() => {
      ...
      {data && ready && (
        <>
          ...
          {!data.latest ? (
            ...
            <>
              <EcbEdition edition={data.latest} />
              <form
                onSubmit={(event) => {
                  event.preventDefault();
                  try {
                    const body = EcbRateReviewSchema.parse({
              ...
              >
                <fieldset disabled={busy || !!pending || !ready}>
                  ...
                  <label htmlFor="ecb-review-status">
                    Policy-rate publication action
                  </label>
                  <select
                    id="ecb-review-status"
                    value={status}
                    onChange={(event) =>
                  ...
                  <label htmlFor="ecb-review-reason">
                    Policy-rate review reason
                  </label>
                  <textarea
                    id="ecb-review-reason"
                    required
                    maxLength={2000}
                    value={reason}
                  ...
                  <button>
                    {onPropose
                      ? 'Propose policy-rate review'
                      : 'Confirm policy-rate review'}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
      ...
      {receipt && (
        <section aria-label="Historical policy-rate review receipt">
      ...
      {reviews && (
        <section aria-label="Policy-rate review history">
          ...
          {reviews.nextBefore && (
            <button
              ...
              onClick={() => {
                ...
                void requestRef
                  .current(
                    `/ops/policy-rates/reviews?before=${reviews.nextBefore}`,
                  )
                  .then(EcbRateReviewsSchema.parse)
                  .then((next) => {
                    if (
      ...
      {runs && (
        <section aria-label="Policy-rate refresh history">
                ...
                {item.sourceHash && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setError('');
                      void requestRef
                        .current(`/ops/policy-rates/retained/${item.requestId}`)
                        .then(EcbRateRetainedSchema.parse)
                        .then((value) => {
                          if (
                            live.current &&
                            !denied.current &&
      ...
      {(raw || rawPending) && (
        <section aria-label="Retained policy-rate response">
          <button
            onClick={() => {
              detailGeneration.current++;
              setRaw(null);
          ...
          {raw && (
            <>
              <p>
                Operator-only raw response · {raw.retrievedAt} ·{' '}
                {raw.sourceHash}
              </p>
              <pre>{raw.body}</pre>
            </>
