...
5. Tap the floating finger → Screenshot + feedback or Feedback only. Crop/cover the app screenshot, type or record, preview and explicitly submit. More → Your feedback keeps it on-device until you enable a reachable HTTPS feedback API. Close/reopen and inspect the pending report. [Feedback configuration](feedback.md) explains receipts, retries, review and deletion; financial records remain separate. More → App settings contains build details.
...
Source changes in the repository do not update an installed offline APK. SOURCES-002 uses the same React source for web and Android, but requires a new public snapshot, web bundle and native APK. The current version is `0.4.0-planning` / code 5, including TEAM-002 allocations, recovery, record reports and dated security identities. Its current APK size, SHA256 and complete web-bundle hash are recorded in `artifacts/android/build.json` and [status](status.md). Install the rebuilt APK over the prior build using the same signing key; do not clear storage. Confirm the actual installed version and `artifacts/android/build.json`, rather than assuming a repository change reached the phone.

The snapshot preserves each article’s original publication/effective date, retrieval time, source and history. Its capture date is packaging metadata, not the publication date of annual observations. The SOURCES-002 capture was verified with all 10 then-existing offline cases. FEEDBACK-001 adds offline feedback acceptance; current results for the final assets are recorded in status. An Android 35 same-key update preserved the existing local holding, and airplane-mode Today→PIB reader→system Back worked. Exact evidence is in [status](status.md); physical-phone acceptance remains open. New source filters, related reading and sourced quizzes work from the captured corpus in airplane mode. Refreshing a source still requires the connected API and a subsequent explicit snapshot/build.
...
Local appasset storage and CDN-origin storage stay isolated. Switching never uploads or merges local accounts/holdings, and remote credentials do not sign into a local profile. Explicitly submitted feedback is the separate opt-in exception: its native queue survives origin changes and sends only to its configured feedback destination while enabled. Use offline mode again to return to local records. Account migration/sync, background push, release signing/Play Store, iOS and WhatsApp remain separate work. No CDN/API deployment is performed by this task.
