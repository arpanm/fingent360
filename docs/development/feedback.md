...
The real API applies an hourly address-bucket limit of 20 new reports/cancellation tombstones (200 globally). Repeated full feedback test runs share that limit; if the server returns 429, keep the report and retry after the next server hour. Do not clear production quotas or other reports to make tests pass. Identical report retries and deletion of an existing report do not consume new-report quota.
