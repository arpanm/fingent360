...
API1521 publishes both the initial and corrected event editions through named proposals: the preparing actor proposes the exact draft head and a different named reviewer approves it. Its shared helper also proves the blocked legacy route and denied proposer self-approval, then returns the actual public revision used by corrected brief preparation. This keeps revision acceptance aligned with the production governance path without weakening authorization.

OFFLINE1520's retained-event fixture applies the complete evidence-graph publication transition before strict parsing: the event is published and all linked context edges are reviewed. The case directly asserts that invariant before using the real offline handler, while retaining its network denial, five-current-point admission, missing-event suppression and source-withdrawal suppression checks. This repairs only invalid fixture setup; production graph validation and offline re-admission remain unchanged.
