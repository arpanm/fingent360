...
## Actual scoped retries — 19 September 2026

Portfolio run1789836377279-19314: format/check succeeded;12 connected failures and5 passes. Saved-reading run1789836492361-20464: format/check succeeded;10 connected failures and1 pass. All22 reported failures contain connection-refused errors for API4104/web5176. No offline stage ran because connected tests failed first. These are unavailable-target failures; the logs do not identify why the servers were absent. Latest commit814796a is the user workflow's saved-reading commit, not successful functional acceptance.

Start databases with `pnpm db:up`, then keep `pnpm dev` running in that terminal until both API and web report ready. Run the two scoped commands above in another terminal. If explicitly set, E2E_API_URL/E2E_WEB_URL must point to that session; otherwise the launcher uses .env. No speculative product changes or assertion changes were made for this infrastructure failure.
