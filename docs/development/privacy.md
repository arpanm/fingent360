...
Goal comparisons retain captured owned goal editions, explicit alternatives and immutable adoption receipts in private export. Adoption creates the normal goal history; it does not rewrite a comparison. All comparison/adoption data is account-scoped on server/device and removed on account deletion; the100comparison limit is explicit and no selective historical rewrite is supported.

Optional v2 record reports retain separately consented copies of selected research connection reasons, exact minimal source/target receipts and capture-time review context. Connection removal does not recall previously issued/downloaded report copies. Individual report deletion physically removes its server/local snapshot/output and leaves the existing metadata-only tombstone; account deletion removes all owned report/connection data. No source article text or provider query is included.

Connection review inbox stores owned target labels, review reasons and version fingerprints, dated notice/check/acknowledgement receipts and no source article text. At most200 resolved notices plus current notices and100 evaluation records remain;1000 operation receipts are retained for30days and new requests reject at unexpired capacity. Pruning never changes financial/connection history. An expired request ID starts a new operation. Private export includes retained records and account deletion cascades server/local inbox state.
