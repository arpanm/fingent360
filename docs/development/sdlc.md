...
The plan is saved in the run's ignored `artifacts/sdlc/<run>/impact-plan.json`, listing baseline, changed paths, reasons and selected files. Positional file patterns are escaped and anchored. Offline selections execute `pnpm android:web` first, then `pnpm android:test` with those files; this tests the packaged web experience, not a native APK/device. Offline failures stop with exact output; their runner does not yet support the connected runner's automatic single-case repair receipt, so no broad agent retry is dispatched. Connected failure repair retains its existing exact-case behavior. An empty documentation-only plan explicitly preserves older E2E evidence; it never records a new E2E pass.
