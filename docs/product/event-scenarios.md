...
Fixed historical RBI/Fed, BLS CPI, BEA original GDP vintage, Infosys earnings/guidance, FIU governance and NSE/CDSL institutional-flow packs are now authored. See the [dated family acceptance matrix](../tasks/EVENT-SCENARIOS-001.md#accepted-scope-reconciliation--2026-09-15) for exact outcomes, source originals versus reconstructed fixtures, and test IDs. Source-bound flow publication uses the existing event/scenario workflow and invalidates dependent reads after source withdrawal. These changes supersede the earlier BLS-only implementation summary.

Real earlier CPI/GDP expectation vintages, general permitted/current source activation and populated causal-policy family mappings/goldens remain incomplete. Numerical surprises need a legitimate earlier published expectation; no source means no surprise. Descriptive source deltas do not establish company/holding/goal effects. No live provider or runtime test acceptance is claimed.
...
## Researched FOMC quarter-fraction adapter — 2026-09-15

The generic exact-token rule now has one explicit versioned exception: an actual FOMC target-range phrase can normalize proper half/quarter mixed fractions to the matching target bound using integer arithmetic. It never converts arbitrary fractions in other event families. Operations can extract models from currently admitted official statement citations with matching publication dates; original fractions and source hashes remain in the stored receipt and displayed conversion explanation. The fixed historical provider captures July31/September18,2024 documents as review-required drafts. Actual historical goldens establish a-0.5 percentage-point corresponding-bound change; live pipeline validation remains pending. See [complete workflow and manual acceptance](../development/fed-policy-pack.md).
