...
OFFLINE081 first retry reached the local static server before the asynchronous on-device fetch transport finished installing after reload. Wait for actual signed-in UI before API helper calls, and use soft cleanup assertions so cleanup cannot mask the original failure. The static server continues to reject every real API request; no transport or authentication bypass was added. Four other newly authored offline cases passed in runoffline-1789847642592-40204.

<!-- sdlc-validation:start -->

## Automated validation

Failed — unresolved bug. [Evidence](../validation/README.md); [bugs](../bugs/README.md). Latest reconciliation: 1789847795826-40501.
<!-- sdlc-validation:end -->
