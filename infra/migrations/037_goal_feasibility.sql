CREATE TABLE app_goal_feasibility (
...
CREATE FUNCTION protect_goal_feasibility() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 ...
 RAISE EXCEPTION 'Assessment is immutable except explicit owned deletion';
END; $$;
CREATE TRIGGER goal_feasibility_immutable BEFORE UPDATE OR DELETE ON app_goal_feasibility FOR EACH ROW EXECUTE FUNCTION protect_goal_feasibility();
