CREATE TABLE app_impact_traces (
...
CREATE FUNCTION protect_impact_trace() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 ...
 RAISE EXCEPTION 'Impact trace is immutable except explicit owned deletion';
END; $$;
CREATE TRIGGER impact_trace_immutable BEFORE UPDATE OR DELETE ON app_impact_traces FOR EACH ROW EXECUTE FUNCTION protect_impact_trace();
