CREATE TABLE broker_upstox_connections (
...
CREATE TABLE broker_upstox_captures (
...
CREATE INDEX broker_upstox_capture_owner ON broker_upstox_captures(user_id,retrieved_at DESC);
CREATE TABLE broker_upstox_events(id uuid PRIMARY KEY,user_id uuid NOT NULL REFERENCES app_users(id) ON DELETE CASCADE,action text NOT NULL,recorded_at timestamptz NOT NULL DEFAULT now());
