CREATE TABLE broker_angel_connections (
...
CREATE TABLE broker_angel_captures (
...
CREATE INDEX broker_angel_capture_owner ON broker_angel_captures(user_id,retrieved_at DESC);
CREATE TABLE broker_angel_events(id uuid PRIMARY KEY,user_id uuid NOT NULL REFERENCES app_users(id) ON DELETE CASCADE,action text NOT NULL,recorded_at timestamptz NOT NULL DEFAULT now());
