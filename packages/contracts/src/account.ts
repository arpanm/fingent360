...
export const AccountSchema = z.strictObject({
  id: z.uuid(),
  username: UsernameSchema,
  consentVersion: z.literal('account-storage-v1'),
  createdAt: z.iso.datetime(),
});
export const CurrentAccountSchema = z.strictObject({
  user: AccountSchema.nullable(),
});
export const WatchlistSchema = z.strictObject({
  indicators: z
    .array(MacroIndicatorSchema)
    .max(2)
    .refine(
...
export const InboxSchema = z.strictObject({
  items: z.array(InboxItemSchema).max(2),
});
