...
export const MacroValueSchema = z
  .string()
  .regex(/^-?(0|[1-9]\d{0,17})(\.\d{1,30})?$/);
export const MacroObservationSchema = z.strictObject({
  id: z.uuid(),
  indicator: MacroIndicatorSchema,
  year: z.number().int().min(1960).max(2200),
  value: MacroValueSchema.nullable(),
  unit: z.literal('annual_percent'),
  country: z.literal('IND'),
  providerUpdatedAt: z.iso.date(),
