...
try {
  ...
  const api = await availablePort(
    env.API_PORT || 4100,
    env.API_HOST || '127.0.0.1',
  );
  const web = await availablePort(
    preview ? env.PREVIEW_PORT || 4173 : env.WEB_PORT || 5173,
    '127.0.0.1',
    new Set([api]),
  );
  const selected = {
  ...
  await run(
    ...
    [
      ...
      '-n',
      'contracts,api,web',
      'pnpm --filter @fingent360/contracts dev',
      'pnpm --filter @fingent360/api dev',
      preview
        ? 'pnpm --filter @fingent360/web preview'
        : 'pnpm --filter @fingent360/web dev',
    ],
    { ...process.env, ...env, ...selected },
  );
} catch (error) {
