...
## Finish one functional story

`pnpm sdlc "Complete account workflow" --story ACCOUNT-001` runs the reviewed connected and offline matrix after normal gates and a local commit. It does not depend on unstaged differences, so a previously committed failure is still selected. Results update docs/validation, per-story records, TODO's validation column and docs/bugs. Skipped or partial coverage never completes a story. See docs/tasks/acceptance.json for initial reviewed matrices; no extra dependency is introduced. Generated post-test tracking changes remain uncommitted.
