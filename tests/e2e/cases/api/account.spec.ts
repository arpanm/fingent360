...
test.describe('Account ownership and watchlists @ACCOUNT-001', () => {
  test('E2E-API-030 register, save, isolate, login and delete', async ({
    request,
    playwright,
    baseURL,
  }) => {
    ...
    try {
      ...
      const first = CurrentAccountSchema.parse(
      ...
      expect(
        (
          await other.post('/api/v1/account/register', {
            headers,
            data: { username: secondName, password, consent: true },
          })
        ).status(),
      ).toBe(201);
      expect(
        WatchlistSchema.parse(
          await (await other.get('/api/v1/account/watchlist')).json(),
        ).indicators,
      ).toEqual([]);
      expect(
        (
          await other.put('/api/v1/account/watchlist', {
            headers,
            data: { indicators: [], userId: first.id },
          })
        ).status(),
      ).toBe(400);
      expect(
      ...
      expect((await request.get('/api/v1/account/watchlist')).status()).toBe(
        401,
      );
      expect(
        (
  ...
  test('E2E-API-031 wrong password, origin and consent reject; cookies are HttpOnly', async ({
    request,
  }) => {
    const username = `e2e_${randomUUID().slice(0, 16)}`;
    const data = { username, password, consent: true };
    expect(
      (await request.post('/api/v1/account/register', { data })).status(),
    ).toBe(403);
    expect(
      (
        await request.post('/api/v1/account/register', {
    ...
    const created = await request.post('/api/v1/account/register', {
      headers,
      data,
    });
    ...
    try {
      ...
      expect(
        (
          await request.delete('/api/v1/account', {
            headers,
            data: { password: 'incorrect-password-123' },
          })
        ).status(),
      ).toBe(401);
      expect(
      ...
      expect(
        (
          await request.put('/api/v1/account/watchlist', {
            headers,
            data: { indicators: ['NY.GDP.MKTP.KD.ZG', 'NY.GDP.MKTP.KD.ZG'] },
          })
        ).status(),
      ).toBe(400);
      await request.post('/api/v1/account/logout', { headers, data: {} });
      expect(
        (
          await request.post('/api/v1/account/login', {
            headers,
            data: { username, password: 'incorrect-password-123' },
          })
        ).status(),
      ).toBe(401);
      expect(
