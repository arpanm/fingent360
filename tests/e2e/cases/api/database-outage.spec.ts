...
test('E2E-API-004 database outage reports 503 @manual-outage @SETUP-001 @SDLC-001', async ({
  request,
}) => {
  const down = process.env.E2E_EXPECT_DOWN;
  test.skip(
    down !== 'postgres' && down !== 'mongodb',
    'User must prepare one database outage and set E2E_EXPECT_DOWN=postgres or mongodb before launching the runner.',
  );
