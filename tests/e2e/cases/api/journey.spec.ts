...
test.describe('Working journey @SLICE-001', () => {
  test('E2E-API-010 catalog identifies every claim as synthetic', async ({
    request,
  }) => {
  ...
  test('E2E-API-011 persisted portfolio, replay, concurrency and immutable reviews', async ({
    request,
  }) => {
    ...
    try {
      ...
      const response = await request.post(`${base}/workspace`, {
        headers,
        data,
      });
      expect(response.status()).toBe(200);
      ...
      expect(
        (
          await request.post(`${base}/workspace`, {
            headers,
            data: { ...data, portfolio: { ...portfolio, cash: '0.00' } },
          })
        ).status(),
      ).toBe(409);
      expect(
      ...
      const reviewResponse = await request.post(`${base}/reviews`, {
        headers,
      ...
      for (const scenario of ['stale', 'conflicting']) {
        const blocked = await request.post(`${base}/reviews`, {
          headers,
          data: { scenario },
        });
        expect(ReviewSchema.parse(await blocked.json()).status).toBe(
          'unable_to_assess',
        );
      }
      expect(
        (
          await request.post(`${base}/workspace`, {
            ...
            data: {
              expectedRevision: 1,
              idempotencyKey: randomUUID(),
              portfolio: { ...portfolio, cash: '2000.00' },
            },
          })
        ).status(),
      ).toBe(200);
      const original = await request.get(`${base}/reviews/${review.id}`, {
        headers,
      });
      expect(ReviewSchema.parse(await original.json()).valuation.total).toBe(
        '2000.00',
      );
      const reload = await request.get(`${base}/workspace`, { headers });
      expect(WorkspaceSchema.parse(await reload.json()).valuation.total).toBe(
        '3000.00',
      );
    } finally {
  ...
  test('E2E-API-012 CSV preview is non-mutating and confirmation reconciles/idempotent', async ({
    request,
  }) => {
    const headers = await session(request);
    try {
      const data = {
        csv: 'instrumentId,quantity\nalpha-air,10\n',
        cash: '1000.00',
        sourceTotal: '2000.00',
      };
      for (const variant of [
        { ...data, sourceTotal: '1.00' },
        { ...data, csv: 'instrumentId,quantity\nalpha-air,10\nalpha-air,10' },
        { ...data, csv: 'instrumentId,quantity\nunknown,10' },
      ]) {
        const response = await request.post(`${base}/previews`, {
          headers,
          data: variant,
        });
        ...
        expect(
          (
            await request.post(`${base}/imports`, {
              headers,
              data: {
      ...
      const imported = await request.post(`${base}/imports`, {
        headers,
        data: confirmation,
      });
      expect(imported.status()).toBe(200);
      expect(WorkspaceSchema.parse(await imported.json()).valuation.total).toBe(
        '2000.00',
      );
      const replay = await request.post(`${base}/imports`, {
        headers,
        data: confirmation,
      });
  ...
  test('E2E-API-013 ownership, invalid input and deletion', async ({
    request,
  }) => {
    const headers = await session(request);
    ...
    try {
      ...
      const response = await request.post(`${base}/reviews`, {
        headers,
        data: { scenario: 'baseline' },
      });
      ...
      expect(
        (
          await request.get(`${base}/reviews/${review.id}`, { headers: other })
        ).status(),
      ).toBe(404);
      const previewResponse = await request.post(`${base}/previews`, {
        headers,
        data: {
          csv: 'instrumentId,quantity\nalpha-air,10',
          cash: '1000.00',
      ...
      expect(
        (await request.get(`${base}/workspace`, { headers })).headers()[
          'cache-control'
        ],
      ).toBe('no-store');
      for (const invalid of [
        ...
        {
          ...portfolio,
          holdings: [{ instrumentId: 'alpha-air', quantity: '-1' }],
        },
        {
          ...portfolio,
          goals: [{ ...portfolio.goals[0], allocationPercent: 101 }],
        },
      ])
        expect(
          (
      ...
      await request.delete(`${base}/workspace`, { headers });
      expect(
        (await request.get(`${base}/workspace`, { headers })).status(),
      ).toBe(401);
    } finally {
      await request.delete(`${base}/workspace`, { headers: other });
      await request.delete(`${base}/workspace`, { headers });
    }
  });
});
