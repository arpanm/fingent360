...
const path = '/api/v1/account/broker-connections/kite';
...
async function connect(request: Parameters<typeof registerRecoverable>[0]) {
  ...
  const callback = await request.post(path + '/complete', {
    ...
    data: {
      state,
      request_token: 'synthetic_request_token',
      status: 'success',
      action: 'login',
    },
    maxRedirects: 0,
...
test('E2E-API-1420 simulated Kite authorization produces actual encrypted capture, exact preview, confirmation and private export @DEV-028 @TEST-SIMULATION', async ({
  request,
  feedbackSandbox,
}) => {
  ...
  try {
    ...
    const encrypted = (
      await db.query(
        'SELECT encrypted_token FROM broker_kite_connections WHERE user_id=$1',
        [owner.id],
      )
    ).rows[0];
...
test('E2E-API-1421 revoke cancels pending consent and real saved preview while confirmed holdings remain @DEV-028 @TEST-SIMULATION', async ({
  request,
  feedbackSandbox,
}) => {
  const owner = await registerRecoverable(request),
  ...
  try {
    ...
    expect(
      (
        await db.query('SELECT 1 FROM broker_kite_captures WHERE user_id=$1', [
          owner.id,
        ])
      ).rowCount,
    ).toBe(0);
...
test('E2E-API-1422 broker parser rejects unsettled duplicates and unsupported fields without float cost arithmetic @DEV-028 @TEST-SIMULATION', async () => {
  const moduleUrl = new URL(
    '../../../../apps/api/dist/kite-provider.js',
    import.meta.url,
  ).href;
  const { parseKiteHoldings } = await import(moduleUrl);
  const fixtureUrl = new URL(
    '../../helpers/kite-simulation.mjs',
    import.meta.url,
  ).href;
  const { syntheticKiteHolding: row } = await import(fixtureUrl);
  expect(
    parseKiteHoldings(JSON.stringify({ status: 'success', data: [row] }))[0]
      .totalCostMinor,
  ).toBe('30038');
  for (const changed of [
    ...
    expect(() =>
      parseKiteHoldings(JSON.stringify({ status: 'success', data: [changed] })),
    ).toThrow();
  expect(() =>
    parseKiteHoldings(JSON.stringify({ status: 'success', data: [row, row] })),
  ).toThrow();
});
test('E2E-API-1424 callback requires the initiating signed-in account and never exchanges on its GET page @DEV-028 @TEST-SIMULATION', async ({
  request,
  playwright,
  feedbackSandbox,
}) => {
...
test('E2E-API-1425 incomplete authorization and expired pending state recover without linking or stale cancellation @DEV-028 @TEST-SIMULATION', async ({
  request,
  feedbackSandbox,
}) => {
  const owner = await registerRecoverable(request),
  ...
  const start = async () => {
    ...
    return new URLSearchParams(
      new URL((await response.json()).loginUrl).searchParams.get(
        'redirect_params',
      )!,
    ).get('state')!;
  };
  try {
    ...
    await db.query(
      "UPDATE broker_kite_connections SET pending_until=now()-interval '1 second' WHERE user_id=$1",
      [owner.id],
    );
    expect((await (await request.get(path)).json()).state).toBe('expired');
    const stored = (
      await db.query(
        'SELECT state_hash,encrypted_token FROM broker_kite_connections WHERE user_id=$1',
        [owner.id],
      )
    ).rows[0];
    ...
    expect(
      (
        await request.post(path + '/complete', {
          headers: authHeaders,
          data: { state: next, request_token: 'synthetic_request_token' },
          maxRedirects: 0,
        })
      ).status(),
    ).toBe(409);
...
test('E2E-API-1426 changed server app key refuses the old vault before any provider request @DEV-028 @TEST-SIMULATION', async () => {
  const moduleUrl = new URL(
    '../../../../apps/api/dist/kite-provider.js',
    import.meta.url,
  ).href;
  const { KiteProvider } = await import(moduleUrl);
  await expect(
    new KiteProvider('new_synthetic_key', 'synthetic_secret').holdings({
      accessToken: 'synthetic_access_token',
      apiKey: 'old_synthetic_key',
      brokerUser: 'SYNTHETIC',
    }),
  ).rejects.toThrow('Kite application changed');
});
