...
test.describe('Real macro source ingestion @DATA-001 @external', () => {
  ...
  test('E2E-API-020 real provider → raw evidence → revisions → persisted API', async ({
    request,
  }) => {
    ...
    for (const indicator of ['NY.GDP.MKTP.KD.ZG', 'FP.CPI.TOTL.ZG']) {
      ...
      expect(
        latest,
        'The real provider should supply at least one non-null annual observation',
      ).toBeTruthy();
      ...
      const evidenceResponse = await request.get(
        `/api/v1/macro/evidence/${latest.sourceHash}`,
      );
      ...
      expect(
        createHash('sha256')
          .update(evidence.url + '\n' + evidence.body)
          .digest('hex'),
      ).toBe(latest.sourceHash);
      expect(Array.isArray(JSON.parse(evidence.body))).toBe(true);
      const history = MacroHistorySchema.parse(
        await (
          await request.get(`/api/v1/macro/${indicator}/history/${latest.year}`)
        ).json(),
      );
      expect(history[0]?.id).toBe(latest.id);
      const again = await request.post('/api/v1/macro/refresh', {
        headers,
        data: { indicator },
        timeout: 45000,
      });
      expect(again.status()).toBe(200);
      const reloaded = MacroDashboardSchema.parse(
        await (await request.get('/api/v1/macro')).json(),
      );
      expect(
        reloaded.sources
          .find((s) => s.indicator === indicator)
          ?.observations.find((o) => o.year === latest.year)?.id,
      ).toBe(latest.id);
    }
  });
  test('E2E-API-021 refresh authorization and URL/schema boundaries', async ({
    request,
  }) => {
    const token = await operatorKey();
    const data = { indicator: 'NY.GDP.MKTP.KD.ZG' };
    expect(
      (await request.post('/api/v1/macro/refresh', { data })).status(),
    ).toBe(401);
    expect(
      (
    ...
    expect(
      (await request.get(`/api/v1/macro/evidence/${'0'.repeat(64)}`)).status(),
    ).toBe(404);
  });
});
