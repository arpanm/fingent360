...
const path = '/api/v1/account/broker-connections/upstox';
...
test('E2E-API-1450 simulated Upstox authorization produces actual encrypted capture, exact preview, confirmation and private export @DEV-028 @TEST-SIMULATION', async ({
  request,
  feedbackSandbox,
}) => {
  ...
  try {
    ...
    const encrypted = (
      await db.query(
        'SELECT encrypted_token FROM broker_upstox_connections WHERE user_id=$1',
        [owner.id],
      )
    ).rows[0];
...
test('E2E-API-1451 revoke cancels pending consent and real saved preview while confirmed holdings remain @DEV-028 @TEST-SIMULATION', async ({
  request,
  feedbackSandbox,
}) => {
  const owner = await registerRecoverable(request),
  ...
  try {
    ...
    expect(
      (
        await db.query(
          'SELECT 1 FROM broker_upstox_captures WHERE user_id=$1',
          [owner.id],
        )
      ).rowCount,
    ).toBe(0);
...
test('E2E-API-1452 broker parser rejects unsettled duplicates and unsupported fields without float cost arithmetic @DEV-028 @TEST-SIMULATION', async () => {
  const moduleUrl = new URL(
    '../../../../apps/api/dist/upstox-provider.js',
    import.meta.url,
  ).href;
  const { parseUpstoxHoldings } = await import(moduleUrl);
  const fixtureUrl = new URL(
    '../../helpers/upstox-simulation.mjs',
    import.meta.url,
  ).href;
  const { syntheticUpstoxHolding: row } = await import(fixtureUrl);
  expect(
    parseUpstoxHoldings(JSON.stringify({ status: 'success', data: [row] }))[0]
      .totalCostMinor,
  ).toBe('30038');
  for (const changed of [
    { ...row, t1_quantity: 1 },
    { ...row, instrument_token: 'BSE_EQ|INE002A01018' },
    { ...row, cnc_used_quantity: 1 },
    { ...row, average_price: 0 },
    { ...row, product: 'MTF' },
    ...
    expect(() =>
      parseUpstoxHoldings(
        JSON.stringify({ status: 'success', data: [changed] }),
      ),
    ).toThrow();
  expect(() =>
    parseUpstoxHoldings(
      JSON.stringify({ status: 'success', data: [row, row] }),
    ),
  ).toThrow();
});
test('E2E-API-1454 callback requires the initiating signed-in account and never exchanges on its GET page @DEV-028 @TEST-SIMULATION', async ({
  request,
  playwright,
  feedbackSandbox,
}) => {
  ...
  try {
    const started = await request.post(path + '/start', {
      ...
      state = url.searchParams.get('state')!,
      data = { state, code: 'synthetic_code' };
    const landing = await other.get(path + '/callback', { params: data });
...
test('E2E-API-1455 incomplete authorization and expired pending state recover without linking or stale cancellation @DEV-028 @TEST-SIMULATION', async ({
  request,
  feedbackSandbox,
}) => {
  const owner = await registerRecoverable(request),
  ...
  try {
    ...
    await db.query(
      "UPDATE broker_upstox_connections SET pending_until=now()-interval '1 second' WHERE user_id=$1",
      [owner.id],
    );
    expect((await (await request.get(path)).json()).state).toBe('expired');
    const stored = (
      await db.query(
        'SELECT state_hash,encrypted_token FROM broker_upstox_connections WHERE user_id=$1',
        [owner.id],
      )
    ).rows[0];
    ...
    expect(
      (
        await request.post(path + '/complete', {
          headers: authHeaders,
          data: { state: next, code: 'synthetic_code' },
          maxRedirects: 0,
        })
      ).status(),
    ).toBe(409);
...
test('E2E-API-1456 Upstox token expires at next 03:30 IST including same morning and exact decimal cost is preserved @DEV-028 @TEST-SIMULATION', async () => {
  const moduleUrl = new URL(
    '../../../../apps/api/dist/upstox-provider.js',
    import.meta.url,
  ).href;
  const { upstoxExpiry, parseUpstoxHoldings } = await import(moduleUrl);
  expect(upstoxExpiry(new Date('2026-09-14T21:00:00Z'))).toBe(
    '2026-09-14T22:00:00.000Z',
  );
  expect(upstoxExpiry(new Date('2026-09-14T22:00:00Z'))).toBe(
  ...
  const raw = JSON.stringify({
    status: 'success',
    data: [{ ...row, quantity: 9999999, average_price: 0 }],
  }).replace('"average_price":0', '"average_price":9007199.25474091');
  expect(parseUpstoxHoldings(raw)[0].totalCostMinor).toBe(
    ((9999999n * 900719925474091n * 100n + 50000000n) / 100000000n).toString(),
  );
});
