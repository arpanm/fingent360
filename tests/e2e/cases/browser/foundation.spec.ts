...
test.describe('Foundation browser @SETUP-001 @SDLC-001 @smoke', () => {
  test('E2E-WEB-001 web connects to the real API', async ({ page }) => {
    await test.step('Load the starter and observe its real API result', async () => {
      await page.goto('/');
      await expect(page.getByRole('heading', { level: 1 })).toContainText('Understand the market.');
      await expect(page.getByRole('status')).toHaveText('API connected');
    });
    await test.step('Show the honest foundation scope', async () => {
      await expect(page.getByText('This starter displays no live market data or investment recommendations.', { exact: false })).toBeVisible();
      await expect(page.getByRole('region', { name: 'Planned experience' }).getByRole('article')).toHaveCount(3);
    });
  });
  ...
  test('E2E-WEB-002 API failure is visible @simulated', async ({ page }) => {
    await test.step('Simulate an unavailable health response without stopping real services', async () => {
      await page.route('**/api/v1/health', route => route.fulfill({ status: 503, contentType: 'application/json', body: '{"status":"unavailable"}' }));
      await page.goto('/');
      await expect(page.getByRole('status')).toHaveText('API unavailable');
    });
  });

  test('E2E-WEB-003 malformed API data cannot appear connected @simulated', async ({ page }) => {
    await page.route('**/api/v1/health', route => route.fulfill({ status: 200, contentType: 'application/json', body: '{"status":"ok","service":"fingent360-api","timestamp":"invalid"}' }));
    await page.goto('/');
    await expect(page.getByRole('status')).toHaveText('API unavailable');
  });

  test('E2E-WEB-004 layout fits and brand works by keyboard', async ({ page }) => {
    await page.goto('/');
    ...
    await test.step('Keyboard users can reach and activate the home link', async () => {
      ...
      await expect(page.getByRole('link', { name: 'fingent360' })).toBeFocused();
