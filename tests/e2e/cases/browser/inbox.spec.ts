...
test.describe('Observation inbox browser @ALERT-001 @external', () => {
  test('E2E-WEB-040 follow and acknowledge a real observation', async ({page,playwright}) => {
    ...
    try {
      const refreshed = await api.post('/api/v1/macro/refresh',{headers:{Authorization:`Bearer ${await operatorKey()}`},data:{indicator:'NY.GDP.MKTP.KD.ZG'},timeout:45000});
    ...
    await page.getByLabel('Username',{exact:true}).fill(`e2e_${randomUUID().slice(0,16)}`);
    ...
    try {
      ...
      await inbox.getByRole('button',{name:'Acknowledge observation',exact:true}).click();
      await expect(inbox).toContainText('Acknowledged');
      await page.reload();
      await expect(inbox).toContainText('Acknowledged');
      await expect(inbox.getByRole('button',{name:'Acknowledge observation',exact:true})).toBeDisabled();
    } finally {
