...
test.describe('Real macro browser @DATA-001 @external', () => {
  test('E2E-WEB-020 refresh real data, inspect evidence and reload', async ({
    page,
  }) => {
    ...
    await card
      ...
      .click();
    await expect(card.getByText(/Latest refresh: succeeded/)).toBeVisible({
      timeout: 60000,
    });
    ...
    await card
      ...
      .click();
    await expect(
      page.getByRole('region', { name: 'Source evidence' }),
    ).toContainText('SHA-256');
    await page.reload();
    await expect(
      page
        .getByRole('article', { name: 'India GDP growth', exact: true })
        .getByText(/Latest refresh: succeeded/),
    ).toBeVisible();
