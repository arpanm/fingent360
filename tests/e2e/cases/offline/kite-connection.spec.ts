...
test('E2E-OFFLINE-1420 broker authorization is unavailable on device without issuing provider requests @DEV-028', async ({
  page,
}) => {
  const providerRequests: string[] = [];
  page.on('request', (r) => {
    if (
      new URL(r.url()).hostname.endsWith('kite.trade') ||
      new URL(r.url()).hostname === 'kite.zerodha.com'
    )
      providerRequests.push(r.url());
  });
  await page.goto('/');
  await expect(page.getByLabel('On-device mode')).toBeVisible();
  const username = 'kite_' + randomUUID().slice(0, 12);
    ...
    async (name) =>
      (
        await fetch('/api/v1/account/register', {
          ...
          body: JSON.stringify({
            username: name,
            password: 'Synthetic-offline-kite-password',
            consent: true,
          }),
        })
      ).status,
