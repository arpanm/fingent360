...
test('E2E-OFFLINE-1450 broker authorization is unavailable on device without issuing provider requests @DEV-028', async ({
  page,
}) => {
  const providerRequests: string[] = [];
  page.on('request', (r) => {
    if (new URL(r.url()).hostname === 'api.upstox.com')
      providerRequests.push(r.url());
  });
  await page.goto('/');
  await expect(page.getByLabel('On-device mode')).toBeVisible();
  const username = 'upstox_' + randomUUID().slice(0, 12);
    ...
    async (name) =>
      (
        await fetch('/api/v1/account/register', {
          ...
          body: JSON.stringify({
            username: name,
            password: 'Synthetic-offline-upstox-password',
            consent: true,
          }),
        })
      ).status,
