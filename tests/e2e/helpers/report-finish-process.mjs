// Invoked only inside API225. A plain Node process loads the actual ESM worker
// method without Playwright's CommonJS transform. Never reads the application .env.
import { createRequire } from 'node:module';
import { ReportsStore } from '../../../apps/api/dist/reports.js';
import { RecordReportSchema } from '../../../packages/contracts/dist/index.js';
...
try {
  ...
  await store.finish(
    input.id,
    input.lease,
    RecordReportSchema.parse(input.report),
  );
} catch {
