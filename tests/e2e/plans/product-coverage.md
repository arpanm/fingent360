...
Bond receipt integrity: API/OFFLINE1982 reject altered canonical source/hash/version, mismatched complete observation/date windows and attached evidence without matching input. Offline replay validates the saved contract and exact input/ID binding. Valid immutable historical receipts remain replayable after editorial withdrawal; new saves still require current admission. No migration; manual `pnpm sdlc "Validate credit receipt integrity" -- --grep "E2E-(API|OFFLINE)-1982"`. Authored, not executed.
