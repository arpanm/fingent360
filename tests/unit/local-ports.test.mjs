...
import { availablePort, databaseUrls, updateLocalEnv, localOrigin } from '../../scripts/local-ports.mjs';
...
test('occupied listener remains alive while a different port is selected', async () => {
  ...
  try {
    ...
    assert.notEqual(await availablePort(chosen,'127.0.0.1',new Set([chosen])),chosen);
  } finally { await new Promise((resolve) => server.close(resolve)); }
});
test('database mapping changes preserve credentials, database and query parameters', () => {
  const result = databaseUrls({ DATABASE_URL:'postgresql://user:fakepass@127.0.0.1:55432/local?sslmode=disable',MONGODB_URI:'mongodb://user:fakepass@127.0.0.1:57017/local?authSource=admin' },55433,57018);
  ...
  assert.throws(() => databaseUrls({DATABASE_URL:'postgresql://remote.example/db',MONGODB_URI:'mongodb://localhost/db'},1,2));
});
test('selected env values update without changing unrelated configuration', async () => {
  ...
  try {
    ...
    await updateLocalEnv({API_PORT:'4101',WEB_ORIGIN:'http://127.0.0.1:5174'},file);
    const content = await readFile(file,'utf8'); const env = parseEnv(content);
    assert.equal(env.API_PORT,'4101'); assert.equal(env.WEB_ORIGIN,'http://127.0.0.1:5174');
    assert.equal(env.SECRET,'fake $ value'); assert.ok(content.startsWith('# preserve'));
