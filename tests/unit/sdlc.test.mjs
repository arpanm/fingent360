...
test('failed checks never stage, commit or run E2E', () => {
  const calls = [];
  assert.throws(() => workflow('example', [], (command, args) => {
    calls.push([command, ...args]);
    return args[0] === 'check' ? 1 : 0;
  }), /Workflow stopped/);
  assert.deepEqual(calls, [['pnpm', 'format'], ['pnpm', 'check']]);
});
...
test('E2E failure occurs after commit and never triggers push or rollback', () => {
  const calls = [];
  assert.throws(() => workflow('message with $literal', [], (command, args) => {
    calls.push([command, ...args]);
  ...
  assert.ok(!calls.some((args) => args.includes('push') || args.includes('reset')));
});
